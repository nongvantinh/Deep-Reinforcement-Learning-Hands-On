...
if __name__ == "__main__":
    random.seed(12345)
    env = gymnasium.envs.toy_text.frozen_lake.FrozenLakeEnv(is_slippery=False, render_mode="human")
    ...
    for iter_no, batch in enumerate(iterate_batches(env, net, BATCH_SIZE)):
        ...
        full_batch, observation, acts, reward_bound = filter_batch(full_batch + batch, PERCENTILE)
        if not full_batch:
            continue
        obs_v = torch.FloatTensor(observation)
